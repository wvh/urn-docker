...
func main() {
	...
	items := map[string]time.Time{
		"banana": now.Add(3 * time.Second),
		"apple":  now.Add(2 * time.Second),
		"pear":   now.Add(4 * time.Second),
	}

	// Create a priority queue, put the items in it, and
	// establish the priority queue (heap) invariants.
	tq := TimeQueue{}
	tq.items = make([]*Item, len(items))
	i := 0
	for value, t := range items {
		tq.items[i] = &Item{
			value: value,
			t:     t,
			index: i,
	...
	//heap.Push(&tq, item)
	tq.Push(item)
	//tq.update(item, item.value, now.Add(5*time.Second))
	tq.update(item, item.value, now.Add(5*time.Second))

	showAll := func(item *Item) bool {
		fmt.Printf("found item %+v\n", item)
		return true
	}

	makeShowN := func(n int) func(*Item) bool {
		i := 0
		return func(item *Item) bool {
			if i < n {
				fmt.Printf("found item (< N) %+v\n", item)
				i++
				return true
			}
			return false
	...
	// closure that caches current time
	makeExpiredNow := func() func(*Item) bool {
		now := time.Now()
		return func(item *Item) bool {
			// !After = Before || Equal
			if !item.t.After(now.Add(3 * time.Second)) {
				fmt.Printf("expired item %+v\n", item)
				return true
			}
			return false
		}
	...
	tq.WalkFn(showAll)
	tq.WalkFn(makeShowN(3))
	tq.WalkFn(makeExpiredNow())
	fmt.Println("-- ")

	tq.PopFn(makeExpiredNow())

	// Take the items out; they arrive in ascending priority order.
	for tq.Len() > 0 {
		//item := heap.Pop(&tq).(*Item)
		item := tq.Pop()
		fmt.Printf("%v:%s\n", item.t.Format("15:04:05"), item.value)
	}
}
*/
